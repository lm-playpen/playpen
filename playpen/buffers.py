...
class BranchingEpisodeBuffer(ConversationLike, PreferenceLike):
    ...
    def reset(self):
        """Clear all collected branching points, e.g. between experiments."""
        self._branching_points = []

    def to_conversational_dataset(self, perspective: Model) -> Dataset:
        """Convert collected branching points to a conversational dataset.

        Note: Since BranchingPoints store sub-trajectories, the returned messages
        only cover the episode up to each divergence point, not the full episode.
        Each branching point a branch passed through produces one dataset entry.

        Args:
            perspective: Only include steps generated by this model
        """
        dataset = []
        for trajectory, episode_score in self._trajectories:
            ...
            dataset.append(dict(messages=messages, reward=episode_score))
    ...
    def to_preference_dataset(self, perspective: Model, data_format="conversational") -> Dataset:
        ...
        for siblings in groups.values():
            if len(siblings) < 2:
                continue  # need at least 2 siblings to form a preference pair
            ...
            if chosen.episode_score == rejected.episode_score:
                continue  # no preference signal if outcomes are identical
            pairs.append({
